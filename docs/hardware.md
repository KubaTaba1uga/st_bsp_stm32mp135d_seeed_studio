...
cannot negotiate USB peripheral role dynamically.
...
VBUS is connected to a transistor which act as a switch and a diod simultanously. It's role is protecting power
supply from backffeeding from other power sources like GPIO power line. The output of the switch is connected 
to PMIC, which further convert it into multiple power sources adequate for CPU.  
Power pipeline looks sth like this: USB C -> Switch -> PMIC -> CPU.  
USB C outputs 5V, as well as switch, PMIC converts 5V to various voltages as needed by CPU but mainly 3.3 and 1.8.
...
role is always a peripheral. Similiarly to USB A this port cannot negotiate OTG role dynamically.
