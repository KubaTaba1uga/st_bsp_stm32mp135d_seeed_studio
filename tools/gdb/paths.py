...
BUILD_PATH = os.path.join(ROOT_PATH, "build", "buildroot", "build")
...
path_source_tf_a     = None 
path_source_op_tee   = None
path_source_u_boot   = None
path_source_linux    = None
dirs = os.listdir(BUILD_PATH)
for dir_ in dirs:
    if dir_.startswith("arm-trusted-firmware"):
        path_source_tf_a = os.path.join(BUILD_PATH, dir_)
    if dir_.startswith("optee-os"):
        path_source_op_tee = os.path.join(BUILD_PATH, dir_)
    if dir_.startswith("uboot"):
        path_source_u_boot = os.path.join(BUILD_PATH, dir_)
    if dir_.startswith("linux"):
        path_source_linux = os.path.join(BUILD_PATH, dir_)

repos = [path_source_tf_a, path_source_op_tee, path_source_u_boot, path_source_linux]
if any(repo is None for repo in repos):
    raise ValueError(f"Repo cannot be None: {repos}")
        
# absolute path of elf files
path_elf_tf_a_bl2    = os.path.join(path_source_tf_a, "build", "stm32mp1", "debug", "bl2", "bl2.elf")
path_elf_op_tee_bl32 = os.path.join(path_source_op_tee, "out", "core", "tee.elf")
path_elf_u_boot      = os.path.join(path_source_u_boot, "u-boot")
path_elf_vmlinux     = os.path.join(path_source_linux, "vmlinux")

# absolute path of fip file
path_fip             = os.path.join(ROOT_PATH, "build", "fip.bin")
